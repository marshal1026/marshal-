...
함수(function)
  (매개변수)를 받아 특정 계산(작업)을 수행하고 결과를 반환하는 구조로 되어있다
...
FROM employees
WHERE LOEWR(last_name) = 'higgins';

-- UPPER()함수 - 믄지열을 대문자로 변환    

SELECT UPPER('higgins') FROM dual;   
/*단순 결과를 출력하고 싶을 때 from dual;를 붙여라*/

-- INITCAP()함수 - 문자열의 첫 글자를 대문자로 변환
SELECT employee_id, last_name, department_id
FROM employees
WHERE last_name = INITCAP('higgins');
/* initcap이 lower을 활용하는 것이 더 효율적*/
...
--SUBSTR() - 지정된 길이의 문자열 추출합니다. 질라줍니다
...
--RPAD() - 오른쪽부터 문자식으로 채운 표현식을 반환합니다.(활용예시 : 주민번호뒷자리 가릴 때)
...
/*
날짜 조작 함수
    MONTHS_BETWEEN(datel, date2) : 두 날짜 간의 월 수를 계산합니다. 
...
SELECT MONTHS_BETWEEN(TO_DATE('2016-12-22', 'YYYY-MM-DD'), TO_DATE('2017-05-22', 'YYYY-MM_DD')) FROM dual;
...
TO_CHAR()함수 - 날짜 또는 숫자를 문자열로 변환
    ...
    A.M. 또는 P.M. - 오전/ 오후를 나타내는 마침표가 있는 자오 표시를반환합니다
...
--NVL2()함수 
--첫 번째 표현식을 검사합니다. 첫번째 표현식이 null이 아니면 두번째 표현식을 반환(표현)합니다. 
--첫 번째 표현식이 null 이면 세 번쨰 표현식이 반환됩니다. 
...
SELECT first_name, LENGTH(first_name) AS exprl, 
...
/* 
CASE 식
    IF-THEN-ELSE 운 작업을 수행하여 조건부 조회를 편리하게 수행하도록 합니다.
*/
